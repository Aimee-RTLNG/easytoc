...
var csrf_token = $('meta[name="csrf-token"]').attr('content'); // Imports
...
}); // ANCHOR Liste de tous les tags possibles dans un menu

var tags_list = ["nav", "ul", "li", "a"]; // ANCHOR Liste WYSIWYG : liste de tous les éléments dynamiques ajoutables
// \t = tabulation,  \n = saut de ligne
...
if (_app__WEBPACK_IMPORTED_MODULE_0__["lang"] == "en") {
  element_types = {
    ...
    "type-special": {
      ...
      "reset-button": "\n\t<input type='reset' value='Réinitialiser' accesskey='r' form='generated-menu'>"
    }
  };
} else {
  element_types = {
...
function getOldContent() {
  ...
  $('#full-table #form-title, #full-table h2,#full-table p,#full-table a,#full-table ol,#full-table ul,#full-table hr').attr('contenteditable', true); // On récupère les paramètres
  // Theme
  ...
  var actual_title = $("#full-table #form-title").text();
  $("#form-creator-title").val(actual_title); // Methode

  var actual_method = $("#generated-menu").attr('method');
  $("#form-creator-method").val(actual_method); // Lien

  var actual_link = $("#generated-menu").attr('action');
...
function addElement(element_type, element_type_name) {
  ...
  var previous_content = $('#content-created-blueprint #full-table').html(); // en fonction du type , différentes actions 
  ...
  if (element_type_name == "reset-button") {
    ...
    $('#content-created-blueprint #full-table').html(previous_content + element_content);
  }
...
.on('click', '#full-table fieldset label, #full-table select option', function (e) {
...
.on('focus', '[contenteditable=true], #full-table input, #full-table select, #full-table textarea, #full-table fieldset label, #full-table select option', function (e) {
  ...
  switch ($(this).data("action")) {
    ...
    case "required":
      if (element_selected_container.hasClass('field-required')) {
        ...
        if ($("#full-table .indicator-required").length == 0) {
